...
root = dom.getroot()
elements = root.xpath('.//EntityResponse:Fragment[not((@schema="http://preservica.com/LegacyXIP") or (@schema="http://preservica.com/ExtendedXIP/v6.2"))]',namespaces=namespaces)
elementCounter = 0
list = []
...
if dublinCore not in list:
	...
	dom = ET.parse(filename)
	root = dom.getroot()
	elements = root.xpath('.//EntityResponse:Fragment[not((@schema="http://preservica.com/LegacyXIP") or (@schema="http://preservica.com/ExtendedXIP/v6.0"))]',namespaces=namespaces)
for element in elements:
	try:
		...
		if "_children" in filename:
			...
			for element in elements:
				try:
					...
					dom2 = ET.parse(newFile)
					root2 = dom2.getroot()
					things = root2.xpath('.//EntityResponse:Fragment[not((@schema="http://preservica.com/LegacyXIP") or (@schema="http://preservica.com/ExtendedXIP/v6.0"))]',namespaces=namespaces)
					elementCounter2 = 0
					for thing in things:
			...
			for element in elements:
				try:
					...
					dom2 = ET.parse(newFile)
					root2 = dom2.getroot()
					things = root2.xpath('.//EntityResponse:Fragment[not((@schema="http://preservica.com/LegacyXIP") or (@schema="http://preservica.com/ExtendedXIP/v6.2"))]',namespaces=namespaces)
					elementCounter2 = 0
					for thing in things:
						try:
							elementals = thing.text
